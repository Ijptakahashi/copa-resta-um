...
// Normaliza nome do time para deduplicação
function normName(n='') {
  return n.toLowerCase()
    .replace(/&/g,'and').replace(/\s+/g,' ')
    .replace('united states','usa').replace('bosnia-herzegovina','bosnia and herzegovina')
    .replace('bosnia & herzegovina','bosnia and herzegovina')
    .replace('czechia','czech republic').replace('türkiye','turkey')
    .replace('côte d\'ivoire','ivory coast').replace("cote d'ivoire",'ivory coast')
    .trim()
}
...
export default function Calendar() {
  ...
  async function handleSync() {
    setSyncing(true); await syncMatches(); await load(); setSyncing(false)
  }
  ...
  return (
    <div style={{padding:'20px 16px 100px',maxWidth:480,margin:'0 auto'}}>
      ...
      {matches.length === 0 ? (
        <div style={{background:'#fff',borderRadius:16,padding:'32px 20px',textAlign:'center',boxShadow:'0 2px 12px rgba(0,0,0,.06)'}}>
          ...
          <div style={{fontSize:13,color:'#6B7280',marginBottom:20}}>Sincronize para buscar os jogos da Copa 2026</div>
          <button onClick={handleSync} disabled={syncing}
            style={{padding:'12px 24px',borderRadius:12,border:'none',background:'linear-gradient(135deg,#1A4731,#2D7A54)',color:'#fff',fontFamily:'Sora',fontSize:13,fontWeight:700,cursor:'pointer',display:'flex',alignItems:'center',gap:8,margin:'0 auto'}}>
            ...
            {dates.map(d=>(
              <button key={d} onClick={()=>setDate(d)}
                style={{
                  ...
                  transition:'all .15s',
                  background: selectedDate===d ? 'linear-gradient(135deg,#1A4731,#2D7A54)' : '#fff',
                  color: selectedDate===d ? '#D6B36A' : '#6B7280',
                  boxShadow: selectedDate===d ? '0 4px 12px rgba(26,71,49,.25)' : '0 1px 4px rgba(0,0,0,.08)',
                }}>
                {formatDay(d)}
              </button>
            ))}
          ...
          return (
            <div key={`${match.id}-${match.home_team}`}
              ...
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
                ...
                {live && (
                  <span style={{display:'flex',alignItems:'center',gap:4,background:'#FEE2E2',color:'#DC2626',
                    padding:'3px 8px',borderRadius:20,fontFamily:'Sora',fontSize:9,fontWeight:700}}>
                    <Zap size={10}/> AO VIVO
                  </span>
                )}
                {fin && (
                  <span style={{display:'flex',alignItems:'center',gap:4,color:'#9CA3AF',fontFamily:'Sora',fontSize:9,fontWeight:700}}>
                    <CheckCircle size={10}/> ENCERRADO
                  </span>
                )}
                {!fin&&!live && (
                  <span style={{display:'flex',alignItems:'center',gap:4,color:'#D6B36A',fontFamily:'Sora',fontSize:11,fontWeight:700}}>
