...
import { FINAL_BRACKET } from '../lib/finalbracket'
...
export default function FinalPick({ player }) {
  ...
  async function load() {
    ...
    try {
      ...
      setAllPicks(picks)
      setMatches(ms)
      setDeadline(finalDeadline(ms))
      setMarketOpen(isFinalOpen(ms))
    } catch (e) {
      setError('Erro ao carregar final: ' + e.message)
    } finally {
      setLoading(false)
    }
  }

  const finalPicks = allPicks.filter(
    p => p.phase === 'final' && p.team_name !== 'no_pick'
  )

  const finalPick = finalPicks[0] || null
  ...
  function findMatchRecord(home, away, bracketId) {
    ...
    //    o FINAL_BRACKET já traz o id de cada jogo.
    if (bracketId != null) {
  ...
  async function selectTeam(teamName, matchRecord, otherTeamName) {
    ...
    if (!marketOpen) {
      setError('O mercado das final já fechou. Sua pick está travada.')
      return
    }
    ...
    if (isSelected) {
      ...
      try {
        await removeFinalPickByMatch(player.id, matchRecord.id)

        setAllPicks(prev =>
          prev.filter(p => !(p.phase === 'final' && p.match_id === matchRecord.id))
        )
      } catch (e) {
    ...
    const qfCountExcludingThisMatch = finalPicks.filter(
      p => p.match_id !== matchRecord.id
    ).length

    const validation = validateFinalPick(
      knockoutPicks,
      teamName,
    ...
    try {
      ...
      setAllPicks(prev => {
        const withoutOldQf = prev.filter(p => p.phase !== 'final')
    ...
    <div style={{ maxWidth: 430, margin: '0 auto', background: '#F8F4EE', minHeight: '100svh' }}>
      ...
      <div style={{ padding: '14px 16px 110px' }}>
        ...
        <div style={{ fontSize: 12, color: '#9A9384', marginTop: 4 }}>
          Escolha 1 seleção entre as 8 classificadas. Não existe lado nas final.
        </div>
        ...
        {deadline && marketOpen && (
          ...
          >
            ...
            <span style={{ fontFamily: 'Sora', fontWeight: 600, fontSize: 12, color: '#C4302B' }}>
              Mercado das final fecha em <DeadlineCountdown deadline={deadline} />
            </span>
          </div>
        )}
        ...
        {deadline && !marketOpen && (
          ...
          >
            ...
            <div style={{ fontSize: 11, color: '#6B6B5E', marginTop: 2 }}>
              Sua pick das final está travada.
            </div>
          </div>
        )}
        ...
        <div
          style={{
            ...
            background: finalPick ? '#0E2417' : '#15291C',
            borderRadius: 12,
            padding: '12px 16px',
            margin: '14px 0',
            border: finalPick ? '1px solid rgba(201,164,74,.4)' : 'none',
          }}
        >
          <div>
            <div
          ...
          <div
            style={{
              ...
              height: 10,
              borderRadius: '50%',
              background: finalPick ? '#C9A44A' : 'rgba(255,255,255,.2)',
            }}
          />
        </div>

        {finalPick && marketOpen && (
          <div
            style={{
              background: '#FBF7EC',
              borderRadius: 10,
          ...
          >
            Você já escolheu sua seleção das final. Para trocar, toque na seleção marcada e remova primeiro.
          </div>
        )}
