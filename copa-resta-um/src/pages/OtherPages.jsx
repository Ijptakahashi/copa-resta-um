...
import { getPlayers, getPlayerPicks, getAllPicks } from '../lib/supabase'
import { computeLives, getFlag } from '../lib/gameLogic'

export function Rankings({ player }) {
  const [data, setData]     = useState([])
  const [stats, setStats]   = useState([]) // team win stats
  const [loading, setLoading] = useState(true)
  const [tab, setTab]       = useState('lives') // lives | teams
  ...
  useEffect(() => {
    async function load() {
      ...
      const ranked = await Promise.all(players.map(async p => {
        ...
        const total   = picks.filter(pk => pk.result !== null).length
      ...
      // Team stats: how often each team won when picked
      const teamMap = {}
      allPicks.filter(pk => pk.result !== null && pk.result !== 'no_pick').forEach(pk => {
        const key = pk.team_name
        if (!teamMap[key]) teamMap[key] = { name: key, wins: 0, total: 0 }
        teamMap[key].total++
        if (pk.result === 'win') teamMap[key].wins++
      })
      const teamStats = Object.values(teamMap)
        .filter(t => t.total >= 1)
        .sort((a,b) => (b.wins/b.total) - (a.wins/a.total) || b.wins - a.wins)
      setStats(teamStats)
      setLoading(false)
    }
    load()
  ...
  return (
    <div className="page">
      ...
      <div style={{display:'flex',gap:'8px',marginBottom:'16px'}}>
        <button onClick={()=>setTab('lives')} className={tab==='lives'?'btn-primary':'btn-secondary'}
          style={{flex:1,padding:'10px',marginTop:0,fontSize:'14px'}}>❤️ Vidas</button>
        <button onClick={()=>setTab('teams')} className={tab==='teams'?'btn-primary':'btn-secondary'}
          style={{flex:1,padding:'10px',marginTop:0,fontSize:'14px'}}>⚽ Seleções</button>
      </div>
      ...
      {tab === 'lives' && (
        <div className="card" style={{padding:0,overflow:'hidden'}}>
          <table className="rank-table">
            ...
            <tbody>
              {data.map((p, i) => (
                <tr key={p.id} className={`${p.eliminated?'eliminated':''} ${i===0?'top1':''}`}
                  style={p.id === player.id ? {background:'var(--gold-light)'} : {}}>
                  <td className="rank-pos">
                    {medals[i] || `#${i+1}`}
                  </td>
                  <td>
                    <span style={{fontWeight: p.id===player.id ? 700 : 400}}>
                      {p.name}{p.id===player.id?' (você)':''}
                    </span>
                    {p.inKnockout && <span style={{fontSize:'10px',marginLeft:4}}>🔥MM</span>}
                  </td>
                  ...
                  <td style={{textAlign:'center'}}>
                    <span className={`badge ${p.eliminated?'badge-dead':'badge-alive'}`}>
                      {p.eliminated?'💀':'✅'}
                    </span>
                  </td>
                </tr>
      ...
      {tab === 'teams' && (
        <div className="card">
          ...
          {stats.length === 0 && <div className="text-muted">Ainda sem dados suficientes.</div>}
          {stats.map((t,i) => (
            <div key={t.name} className="stat-row">
              <div style={{display:'flex',alignItems:'center',gap:'8px'}}>
                ...
                <div>
                  ...
                  <div style={{fontSize:'11px',color:'var(--gray-dark)'}}>{t.wins}/{t.total} picks</div>
                </div>
              </div>
              <div style={{textAlign:'right'}}>
                <div style={{fontWeight:700,color:'var(--green-dark)',fontSize:'16px'}}>
                  {Math.round(t.wins/t.total*100)}%
                </div>
...
export function AllPicks() {
  ...
  const { getMatches } = require('../lib/supabase') // handled via import below

  useEffect(() => {
    import('../lib/supabase').then(async ({ getAllPicks: gp, getMatches: gm }) => {
      const [picks, ms] = await Promise.all([gp(), gm()])
      setAllPicks(picks); setMatches(ms); setLoading(false)
    })
  }, [])
  ...
  return (
    <div className="page">
      ...
      <div className="card">
        ...
        <select style={{width:'100%',padding:'8px 10px',border:'2px solid var(--gray-mid)',borderRadius:'8px',fontSize:'14px'}}
          value={selectedDate} onChange={e=>setDate(e.target.value)}>
...
export function Inventory({ player }) {
  ...
  useEffect(() => {
    import('../lib/supabase').then(({ getPlayerPicks }) => {
      getPlayerPicks(player.id).then(p => { setPicks(p); setLoading(false) })
    })
  }, [player.id])

  if (loading) return <div className="loading">🗂️ Carregando...</div>

  // Build inventory from picks
  const usedTeams = {}
  ...
  const inventory = Object.values(usedTeams).map(t => {
    const burned   = t.results.includes('loss')
    const unlocked = !burned && t.results.length > 0
    return { ...t, status: burned ? 'burned' : unlocked ? 'unlocked' : 'used' }
  })
  const burned   = inventory.filter(t=>t.status==='burned')
  const unlocked = inventory.filter(t=>t.status==='unlocked')
  const used     = inventory.filter(t=>t.status==='used') // pending result
  ...
  return (
    <div className="page">
      ...
      {burned.length > 0 && (
        <div className="card">
          <div className="card-header">🔴 Queimadas — não pode usar nunca mais</div>
          <div className="inventory-grid">
            {burned.map(t=><TeamChip key={t.id} team={t} cls="inv-burned" icon="💀"/>)}
          </div>
        </div>
      )}
      ...
      {unlocked.length > 0 && (
        <div className="card">
          <div className="card-header">🔵 Desbloqueadas — pode reutilizar</div>
          <div className="inventory-grid">
            {unlocked.map(t=><TeamChip key={t.id} team={t} cls="inv-unlocked" icon="🔓"/>)}
          </div>
        </div>
      )}
      ...
      {used.length > 0 && (
        <div className="card">
          <div className="card-header">⏳ Resultado pendente</div>
          <div className="inventory-grid">
            {used.map(t=><TeamChip key={t.id} team={t} cls="inv-available" icon="⏳"/>)}
          </div>
        </div>
      )}

      <div className="card">
        <div className="card-header">⚪ Disponíveis</div>
        <div className="text-muted" style={{fontSize:'13px'}}>
          Todas as seleções que você nunca usou estão disponíveis. Use com sabedoria! 🧠
        </div>
        <div style={{marginTop:'8px',fontSize:'12px',color:'var(--gray-dark)'}}>
          {48 - inventory.length} seleções intactas restantes.
        </div>
      </div>
    </div>
  )
...
// Need import at top for computeLives in Inventory
import { computeLives, getFlag } from '../lib/gameLogic'
