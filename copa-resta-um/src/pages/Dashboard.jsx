...
const PHASE_INFO = {
  ...
  final: { label: 'Final', route: '/final', reveal: 'PICKS DA FINAL', description: '1 pick única — Argentina x Espanha' },
}
...
function phaseFromMatchesAndPicks(matches, playerPicks) {
  ...
  const pickedFinal = playerPicks.some(p => p.phase === 'final')
  ...
  if (hasFinal || pickedFinal) return 'final'
...
function deadlineForPhase(phase, matches, todayMatches) {
  if (phase === 'final') return finalDeadline(matches)
...
function isPhaseOpen(phase, matches, todayMatches) {
  if (phase === 'final') return isFinalOpen(matches)
...
function playerSlots(playerId, allPicks, phase) {
  ...
  if (phase === 'qf' || phase === 'sf' || phase === 'final') return [picks[0] || null]
...
function MyKnockoutCard({ phase, picks, open, onClick }) {
  ...
  let content
  if (phase === 'qf' || phase === 'sf' || phase === 'final') {
        ...
        {content.map(item => {
          ...
          return (
            <div key={item.label} style={{
              flex: (phase === 'qf' || phase === 'sf' || phase === 'final') ? '0 1 220px' : 1,
    ...
    <Card>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 14, justifyContent: 'center' }}>
        {allPlayers.map(player => {
          ...
          return (
            <div key={player.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, width: (phase === 'qf' || phase === 'sf' || phase === 'final') ? 72 : 88, opacity: eliminated ? .6 : 1 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4, width: '100%' }}>
                {slots.map((pick, index) => {
                  const separator = !isGroups && phase !== 'qf' && phase !== 'sf' && phase !== 'final' && ((phase === 'r16' && index === 1) || (phase === 'r32' && index === 2))
                  return (
                    <span key={index} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                      {separator && <span style={{ width: 1, height: 18, background: 'rgba(0,0,0,.12)' }} />}
                      {revealed && pick && pick.team_name !== 'no_pick' ? (
                        <FlagBubble team={pick.team_name} size={(phase === 'qf' || phase === 'sf' || phase === 'final') ? 28 : 22} />
                      ) : (
                        <span style={{
                          width: (phase === 'qf' || phase === 'sf' || phase === 'final') ? 28 : 22,
                          height: (phase === 'qf' || phase === 'sf' || phase === 'final') ? 28 : 22,
                          borderRadius: '50%',
