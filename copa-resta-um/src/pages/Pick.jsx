...
import { ArrowLeft, Info, Lock, RefreshCw, CheckCircle, AlertTriangle } from 'lucide-react'
      ...
      <div style={{padding:'14px 16px 96px'}}>
        ...
        <div style={{marginBottom:14}}>
          <div style={{fontFamily:'Sora',fontWeight:800,fontSize:30,color:'#1A3D28',
            ...
            {isPast?'PICK DO DIA':canChange?'ALTERAR PICK':canPick?'MAKE YOUR\nPICK':'MAKE YOUR\nPICK'}
          </div>
          <div style={{fontSize:12,color:'#9CA3AF',marginTop:5,fontFamily:'Inter'}}>
            {isPast?'Pick encerrado para este dia':
             ...
             canPick?'Selecione um time para vencer hoje':
             deadline&&!isPast?`Deadline: ${fmtDeadline(deadline)} BRT`:''}
          </div>
        </div>

        {/* Deadline countdown */}
        {deadline && !isPast && !dayPick && (
          ...
          <div style={{background:canChange?'rgba(26,61,40,.04)':'#fff',borderRadius:14,
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:11,color:'#9CA3AF',marginTop:2,fontFamily:'Inter'}}>
                {canChange?'Sua pick atual — pode alterar':'Pick confirmada ✓'}
              </div>
            </div>
            {canChange?(
              <span style={{fontFamily:'Sora',fontSize:9,fontWeight:700,
                color:'#C9A44A',background:'#FBF5E6',padding:'4px 10px',borderRadius:12,
                letterSpacing:'.06em'}}>ALTERAR</span>
            ):(
              isPast&&dayPick.result?(
                ...
                <span style={{fontFamily:'Sora',fontSize:9,fontWeight:700,
                  color:'#1A3D28',background:'#EBF5EE',padding:'4px 10px',borderRadius:12,letterSpacing:'.06em'}}>LOCKED ✓</span>
              )
            )}
          </div>
        )}
        ...
        {!canPick && !isPast && !dayPick && dayMs.length > 0 && (
          <div style={{background:'#FEF0EF',borderRadius:14,padding:'20px',textAlign:'center',
            ...
            <div style={{fontFamily:'Sora',fontWeight:700,color:'#C4302B',fontSize:15}}>DEADLINE ENCERRADO</div>
        ...
        {/* Eliminated */}
        {eliminated && dayMs.length > 0 && (
          <div style={{background:'#FEF0EF',borderRadius:14,padding:'24px',textAlign:'center',
            border:'1px solid rgba(196,48,43,.15)',marginBottom:14}}>
            <div style={{fontFamily:'Sora',fontWeight:800,fontSize:18,color:'#C4302B'}}>💀 ELIMINADO</div>
            <div style={{fontSize:12,color:'#9CA3AF',marginTop:4}}>Torça pelos sobreviventes!</div>
          </div>
        )}

        {/* Match cards — pick form */}
        ...
        {canPick && !eliminated && dayMs.length > 0 && (
          <>
            <div style={{display:'flex',gap:10,background:'#fff',borderRadius:12,
              padding:'12px 14px',marginBottom:12,border:'1px solid rgba(0,0,0,.07)'}}>
              <ShieldIcon active size={20}/>
              <div>
                <div style={{fontFamily:'Sora',fontWeight:600,fontSize:12,color:'#1A1A1A'}}>
                  {canChange?'Altere sua pick antes do deadline':'Pick one team to win.'}
                </div>
                <div style={{fontSize:11,color:'#9CA3AF',marginTop:2,fontFamily:'Inter'}}>
                  Deadline: {deadline?fmtDeadline(deadline):''} BRT (30 min antes do 1º jogo)
                </div>
              </div>
            </div>
            ...
            <button onClick={handleSubmit} disabled={!selected || submitting}
              ...
              {submitting?'CONFIRMANDO...':selected?`${canChange?'ALTERAR':'CONFIRMAR'} — ${selected.teamName}`:'SELECIONE UM TIME ACIMA'}
            </button>
            <div style={{textAlign:'center',fontSize:11,color:'#9CA3AF',marginTop:8,fontFamily:'Inter'}}>
              {canChange?'Picks podem ser alteradas até o deadline':'Picks são finais após o deadline.'}
            </div>
          </>
        )}
        ...
        <div style={{display:'flex',alignItems:'center',gap:4,marginTop:16}}>
          {Array.from({length:maxL}).map((_,i)=>(
            <ShieldIcon key={i} active={i<lives} size={18}/>
          ))}
