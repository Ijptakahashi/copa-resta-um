...
import { SF_BRACKET } from '../lib/sfbracket'
...
export default function SFPick({ player }) {
  ...
  async function load() {
    ...
    try {
      ...
      setAllPicks(picks)
      setMatches(ms)
      setDeadline(sfDeadline(ms))
      setMarketOpen(isSfOpen(ms))
    } catch (e) {
      setError('Erro ao carregar semi: ' + e.message)
    } finally {
      setLoading(false)
    }
  }

  const sfPicks = allPicks.filter(
    p => p.phase === 'sf' && p.team_name !== 'no_pick'
  )

  const sfPick = sfPicks[0] || null
  ...
  function findMatchRecord(home, away, bracketId) {
    ...
    //    o SF_BRACKET já traz o id de cada jogo.
    if (bracketId != null) {
  ...
  async function selectTeam(teamName, matchRecord, otherTeamName) {
    ...
    if (!marketOpen) {
      setError('O mercado das semi já fechou. Sua pick está travada.')
      return
    }
    ...
    if (isSelected) {
      ...
      try {
        await removeSfPickByMatch(player.id, matchRecord.id)

        setAllPicks(prev =>
          prev.filter(p => !(p.phase === 'sf' && p.match_id === matchRecord.id))
        )
      } catch (e) {
    ...
    const qfCountExcludingThisMatch = sfPicks.filter(
      p => p.match_id !== matchRecord.id
    ).length

    const validation = validateSfPick(
      knockoutPicks,
      teamName,
    ...
    try {
      ...
      setAllPicks(prev => {
        const withoutOldQf = prev.filter(p => p.phase !== 'sf')
    ...
    <div style={{ maxWidth: 430, margin: '0 auto', background: '#F8F4EE', minHeight: '100svh' }}>
      ...
      <div style={{ padding: '14px 16px 110px' }}>
        ...
        <div style={{ fontSize: 12, color: '#9A9384', marginTop: 4 }}>
          Escolha 1 seleção entre as 8 classificadas. Não existe lado nas semi.
        </div>
        ...
        {deadline && marketOpen && (
          ...
          >
            ...
            <span style={{ fontFamily: 'Sora', fontWeight: 600, fontSize: 12, color: '#C4302B' }}>
              Mercado das semi fecha em <DeadlineCountdown deadline={deadline} />
            </span>
          </div>
        )}
        ...
        {deadline && !marketOpen && (
          ...
          >
            ...
            <div style={{ fontSize: 11, color: '#6B6B5E', marginTop: 2 }}>
              Sua pick das semi está travada.
            </div>
          </div>
        )}
        ...
        <div
          style={{
            ...
            background: sfPick ? '#0E2417' : '#15291C',
            borderRadius: 12,
            padding: '12px 16px',
            margin: '14px 0',
            border: sfPick ? '1px solid rgba(201,164,74,.4)' : 'none',
          }}
        >
          <div>
            <div
          ...
          <div
            style={{
              ...
              height: 10,
              borderRadius: '50%',
              background: sfPick ? '#C9A44A' : 'rgba(255,255,255,.2)',
            }}
          />
        </div>
        ...
        {sfPick && marketOpen && (
          ...
          >
            Você já escolheu sua seleção das semi. Para trocar, toque na seleção marcada e remova primeiro.
          </div>
        )}
